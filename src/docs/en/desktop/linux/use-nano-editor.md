# Using the Nano Editor

1. Open the Ubuntu terminal.
   ![Screenshot](img/use-nano-editor-01.webp)

2. Create a new file and open it:

      ```bash
      nano Dockerfile
      ```
   ![Screenshot](img/use-nano-editor-02.webp)

4. Write your content, then press **Ctrl + O** to save the file.
   ![Screenshot](img/use-nano-editor-03.webp)

5. Press **Enter** to confirm the file name.
   ![Screenshot](img/use-nano-editor-04.webp)

6. Press **Ctrl + X** to exit.
   ![Screenshot](img/use-nano-editor-05.webp)
