# Editing an Image with an External Editor

Configure an external image editor in XnView MP.

## Steps

1. Right-click the image, then select **Open with → Configure programs**.

    ![Screenshot](img/edit-image-with-external-editor-01.webp)

2. Click **Add**.

    ![Screenshot](img/edit-image-with-external-editor-02.webp)

3. Select the **.exe** file of the image editor.

    ![Screenshot](img/edit-image-with-external-editor-03.webp)

4. Click **OK**.

    ![Screenshot](img/edit-image-with-external-editor-04.webp)

5. Right-click the image, then select **Open with**, and choose the editor.