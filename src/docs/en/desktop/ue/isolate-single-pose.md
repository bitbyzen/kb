...
[Creating a Pose Asset from an Animation Sequence](create-pose-asset-from-animation.md) using **Current Animation** generates one pose per key in the source animation. A hand-keyed animation with just a start and end key produces two poses, easy to clean up — but a densely-sampled animation produces one pose per frame instead: 76 poses (`Pose_0` through `Pose_75`) for a 75-frame clip, not the single named pose you usually want. Clean this up by deleting the poses you don't need and renaming the one you're keeping.
